...
class StaffController extends Controller
{
    ...
    public function index(Request $request,Builder $htmlbuilder)
    {
        ...
        ->addColumn([
            ...
            if(Auth::user()->hasRole('admin') || Auth::user()->hasRole('staff-readwrite'))
{
$html=$html
        ->addColumn([
            'defaultContent' => '',
      ...
      if($request->input('roles') && count($roles)>0)
        {
            ...
            // {
            //     $client_role = $request->input('client');
            //     $staff_role = $request->input('staff');
            //     $role_clients=Role::where('name',$client_role)->first();
            //     $role_staff=Role::where('name',$staff_role)->first();
            //     $user->roles()->attach($role_clients);
